...
def reduce_tag(l):
    ...
    for word in l:
        if '*' in word:
            ...
            # This word was a false-negative PN.  Tag with something
            # and continue.

            r.append(FALSE_NEGATIVE)
            continue
...
def find_ngrams(l):
    ...
    for n in range(len(l)):
        ...
        for q in zip(*[ iter(l) ]*n):
            ...
            # Return a list generated from the tuple if it contains a
            # false negative tag.

            if FALSE_NEGATIVE in q:
                yield list(q)
...
def parse_mega_corpus(args):
    ...
    # a false-negative PN (key) and the number of times this pattern has
    # been seen (value).
    ...
    for key in d:
        ...
        for i in range(n):
            ...
            # see if we have a false-negative PN (that is, the word is
            # tagged with *PN* but not also $PN$).  If so, investigate
            # its context.

            if token.endswith('*PN*'):

                # This is a false-negative PN.  Build up a context window
                # around it and analyze the tag patterns.
